...
Kubernetes নোডগুলোর মধ্যে শত শত পোর্টে যোগাযোগ করে (API Server: 6443, kubelet: 10250, etcd: 2379-2380 ইত্যাদি)। UFW চালু থাকলে এই যোগাযোগ ব্লক হয়ে যায়, ফলে ক্লাস্টার তৈরি হয় না বা অস্থির আচরণ করে। ল্যাব/ডেভ এনভায়রনমেন্টে UFW বন্ধ করাই সহজ ও দ্রুত সমাধান।
...
## ধাপ ४ — Kubernetes রিপোজিটরি যোগ করা
...
```bash
# Ubuntu এর containerd প্যাকেজ ইন্সটল করুন
sudo apt-get update
sudo apt-get install -y containerd

# containerd কনফিগার করুন
sudo mkdir -p /etc/containerd
sudo containerd config default | sudo tee /etc/containerd/config.toml
sudo sed -i 's/SystemdCgroup = false/SystemdCgroup = true/g' /etc/containerd/config.toml

# containerd রিস্টার্ট ও এনাবল করুন
...
Kubernetes-এর পডগুলো একে অপরের সাথে এবং বাইরের জগতের সাথে যোগাযোগ করতে পারে। `ip_forward` না চালু থাকলে:
- ❌ পডগুলো পরস্পর কথা বলতে পারে না
- ❌ পডগুলো ইন্টারনেটে যেতে পারে না
- ❌ Service বাইরে থেকে অ্যাক্সেস করা যায় না
- ❌ kubeadm join ফেইল করে

`ip_forward` হলো লিনাক্স কার্নেলের একটি প্যারামিটার যা নেটওয়ার্ক ট্রাফিককে এক ইন্টারফেস থেকে অন্য ইন্টারফেসে ফরওয়ার্ড করার অনুমতি দেয়। Kubernetes-এর জন্য এটি **অত্যন্ত জরুরি**।

| কমান্ড/অপশন | মানে কী |
|---|---|
| `sysctl -w` | কার্নেল প্যারামিটার তাৎক্ষণিকভাবে পরিবর্তন করে |
| `net.ipv4.ip_forward=1` | IP ফরওয়ার্ডিং চালু করে |
| `sysctl -p` | sysctl.conf ফাইল থেকে কনফিগ রিলোড করে |
| `sysctl --system` | সব sysctl কনফিগ ফাইল রিলোড করে |
...
#### ১. তাৎক্ষণিকভাবে ip_forward চালু করুন

```bash
# root ইউজার হয়ে
sudo su -

# ip_forward চালু করুন (তাত্ক্ষণিক)
echo 1 > /proc/sys/net/ipv4/ip_forward

# অথবা sysctl কমান্ড দিয়ে
sysctl -w net.ipv4.ip_forward=1

# চেক করুন
cat /proc/sys/net/ipv4/ip_forward
# আউটপুট: 1 (হতে হবে)
. স্থায়ীভাবে ip_forward সেট করুন (রিবুটের পরেও থাকবে)
bash
# পদ্ধতি ১: sysctl.conf ফাইলে যোগ করুন
echo "net.ipv4.ip_forward = 1" >> /etc/sysctl.conf
sysctl -p

# পদ্ধতি ২: আলাদা কনফিগ ফাইল তৈরি করুন (সুপারিশকৃত)
echo "net.ipv4.ip_forward = 1" > /etc/sysctl.d/99-ipforward.conf
sysctl --system

```

## ধাপ ९ — ক্লাস্টার তৈরি করো (kubeadm init)
...
```bash
sudo kubeadm init \
  --control-plane-endpoint "192.168.0.99:6443" \
  --upload-certs \
...
## ধাপ११ — Flannel নেটওয়ার্ক প্লাগইন ইন্সটল
